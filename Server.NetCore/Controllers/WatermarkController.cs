...
namespace Server.NetCore.Controllers
{
	public class WatermarkController : BaseController
	{
		...
		{
			using var db = SugarContext.GetInstance();
			var watermarks = db.Queryable<SYS_USER_LIKE, WATERMARK_PROPERTY, SYS_USER>((s, w, su) => new(
				JoinType.Inner, s.RESOURCE_ID == w.ID,
				JoinType.Inner, w.USER_ID == su.ID
				)).Where(s => s.USER_ID == userId).Select((s, w, su) => new
				{
					WatermarkId = w.ID,
					UserId = w.USER_ID,
					Name = w.NAME,
					...
					STATE = w.STATE == "A",
					UserDisplayName = su.DISPLAY_NAME
				}).ToList();
			return watermarks;
		}
